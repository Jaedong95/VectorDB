'''
Finger rule-book에 대해 처리하는 코드. PDF 파일 로드 - 텍스트, 이미지 및 테이블 추출 - 파일로 저장하는 과정으로 이루어진다.
'''
import os
import argparse
import tiktoken
...
def main(args):
    ...
    for p_idx in range(len(pages)):
        ...
        # pages[p_idx].metadata['tok_chunk_size'] = tok_chunk_size
        # print(pages[p_idx].metadata)
        for idx in range(len(texts)):
            ...
            # meta_list.append(pages[p_idx].metadata)
    # print(f'len txt: {len(txt_list)}, len metadata: {len(meta_list)}')
    print(f'len txt: {len(txt_list)}, len chunk: {len(chunk_size_list)}')
    '''
    tiktoken usage
    # print(tokenizer.encode(texts[1].page_content))
    # result = tokenizer.encode(texts[1].page_content)
    # print([tokenizer.decode_single_token_bytes(token) for token in result])
    # print(tiktoken_len(texts[1].page_content))    
    '''
    # print(f'texts[0]: {texts[0]}', end='\n\n')
    # print(f'texts[1]: {texts[1]}')
    # print(f'len texts: {len(texts)}')
    finger_df = pd.DataFrame(zip(txt_list, source_list, page_list, chunk_size_list), columns=['text', 'source', 'page_no', 'chunk_size'])
    finger_df.to_csv(os.path.join(args.output_dir, f'{args.file_name.split(".")[0]}.csv'))
